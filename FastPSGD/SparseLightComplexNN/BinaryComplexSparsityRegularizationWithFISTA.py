...
class ComplexSparsityRegularization(Layer):
    ...
    def on_batch_end(self):
        t = self.t
        Weights = K.get_value(self.weights[0])
        weightsUpdate = Weights - self.lastIterationWeights#ST gradient
        ...
        temp[np.shape(temp)[0]//2:] = temp[:np.shape(temp)[0]//2]# complex part == real part     
        temp = SoftSign(temp, mu)#FISTA step2
        temp = np.clip(temp, -1, 1)#clip
        scale = 1/(np.max(abs(temp)))#scaling
        newfullPrecisionWeights = temp * scale#t+1 FP weight
        BinaryWeights = HardSign(newfullPrecisionWeights, self.threshold)#t+1 Bin weight
